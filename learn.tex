...
Contrary to batch normalization, \emph{layer normalization} is only applied to one observation at a time.
Consequently both the offset and the scaling factor are scalars. For an $n$-dimensional vector $x$, 
...
\subsubsection*{Properties of Cross-Entropy}
\begin{itemize}
    \item Maximizing the predictive probability of $Q$ for 
    distribution $P$,  (i.e., $E_{x \sim P} [\log (q(x))]$);
    \item Minimizing cross-entropy $\mathrm{CE} (P, Q)$;
    \item Minimizing the KL divergence $D_{\mathrm{KL}}(P\|Q)$;
\end{itemize}
